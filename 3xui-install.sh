...
os_version=$(grep "^VERSION_ID" /etc/os-release | cut -d '=' -f2 | tr -d '"' | tr -d '.')
    ...
    # local config_webBasePath="qwerABCD"
    # local config_account="admin"
    # local config_password="6yhn7ujm^&"
    # local config_port="5678"

while [[ $# -gt 0 ]]; do
  case $1 in
    --config_webBasePath)
      config_webBasePath=$2
      shift 2
      ;;
    --config_account)
      config_account=$2
    ...
    --config_port)
      ...
      ;;                
    --help)
      echo "Usage: $0 --config_webBasePath <path> --config_account <user> --config_password <password> --config_port <port>"
      exit 0
      ;;
    *)
